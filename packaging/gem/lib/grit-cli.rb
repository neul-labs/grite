...
module GritCli
  ...
  class << self
    ...
    def download_binary(dest_dir)
      plat = platform
      ext = archive_ext
      archive_name = "grit-#{VERSION}-#{plat}#{ext}"
      url = "https://github.com/#{REPO}/releases/download/v#{VERSION}/#{archive_name}"

      puts "Downloading grit v#{VERSION} for #{plat}..."
      ...
      Dir.mktmpdir do |temp_dir|
        ...
        extracted_dir = Dir.glob(File.join(temp_dir, 'grit-*')).find { |d| File.directory?(d) }
      ...
      puts "Successfully installed grit to #{dest_dir}"
    end
