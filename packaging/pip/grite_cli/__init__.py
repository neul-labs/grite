...
def get_binary_path(name: str = "grite") -> Path:
...
def download_binary(dest_dir: Path) -> None:
    ...
    plat = get_platform()
    ext = get_archive_ext()
    archive_name = f"grite-{__version__}-{plat}{ext}"
    url = f"https://github.com/{REPO}/releases/download/v{__version__}/{archive_name}"

    print(f"Downloading grite v{__version__} for {plat}...")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        extracted_dirs = [d for d in temp_path.iterdir() if d.is_dir() and d.name.startswith("grite-")]
        ...
        # Copy binaries
        for binary in ["grite", "grite-daemon"]:
    ...
    print(f"Successfully installed grite to {dest_dir}")


def main() -> None:
    """Entry point for grite command."""
    binary = get_binary_path("grite")
    os.execv(str(binary), [str(binary)] + sys.argv[1:])


def main_daemon() -> None:
    """Entry point for grite-daemon command."""
    binary = get_binary_path("grite-daemon")
    os.execv(str(binary), [str(binary)] + sys.argv[1:])
