...
/// Create a new issue.
pub fn issue_create(ctx: &GriteContext, opts: &IssueCreateOptions) -> Result<IssueCreateResult, GriteError> {
...
/// List issues.
pub fn issue_list(ctx: &GriteContext, opts: &IssueListOptions) -> Result<IssueListResult, GriteError> {
    let store = ctx.open_store()?;

    let state_filter = opts.state.as_ref().map(|s| {
...
/// Show issue details.
pub fn issue_show(ctx: &GriteContext, opts: &IssueShowOptions) -> Result<IssueShowResult, GriteError> {
    let store = ctx.open_store()?;
    ...
    Ok(IssueShowResult { issue: proj, events })
}

/// Update an issue.
pub fn issue_update(ctx: &GriteContext, opts: &IssueUpdateOptions) -> Result<IssueUpdateResult, GriteError> {
    if opts.title.is_none() && opts.body.is_none() {
        return Err(GriteError::InvalidArgs("Either --title or --body must be provided".to_string()));
    }
    ...
    let issue_id = store.resolve_issue_id(&opts.issue_id)?;
    let existing = store.get_issue(&issue_id)?
        .ok_or_else(|| GriteError::NotFound(format!("Issue {} not found", opts.issue_id)))?;
...
pub fn issue_comment(ctx: &GriteContext, opts: &IssueCommentOptions) -> Result<IssueCommentResult, GriteError> {
    let _guard = LockGuard::acquire(ctx, &opts.issue_id, opts.acquire_lock)?;
    ...
    let issue_id = store.resolve_issue_id(&opts.issue_id)?;
    let _existing = store.get_issue(&issue_id)?
        .ok_or_else(|| GriteError::NotFound(format!("Issue {} not found", opts.issue_id)))?;

    let ts = current_ts();
    let kind = EventKind::CommentAdded { body: opts.body.clone() };
    let event_id = compute_event_id(&issue_id, &actor, ts, None, &kind);
    let event = Event::new(event_id, issue_id, actor, ts, None, kind);
    let event = ctx.sign_event(event);
...
/// Close an issue.
pub fn issue_close(ctx: &GriteContext, opts: &IssueStateOptions) -> Result<IssueStateResult, GriteError> {
    let _guard = LockGuard::acquire(ctx, &opts.issue_id, opts.acquire_lock)?;
...
/// Reopen an issue.
pub fn issue_reopen(ctx: &GriteContext, opts: &IssueStateOptions) -> Result<IssueStateResult, GriteError> {
    let _guard = LockGuard::acquire(ctx, &opts.issue_id, opts.acquire_lock)?;
...
pub fn issue_label(ctx: &GriteContext, opts: &IssueLabelOptions) -> Result<IssueLabelResult, GriteError> {
    let _guard = LockGuard::acquire(ctx, &opts.issue_id, opts.acquire_lock)?;
    ...
    let kind = if !opts.add.is_empty() {
        EventKind::LabelAdded { label: opts.add[0].clone() }
    } else if !opts.remove.is_empty() {
        EventKind::LabelRemoved { label: opts.remove[0].clone() }
    } else {
        return Err(GriteError::InvalidArgs("No labels to add or remove".to_string()));
    };
...
pub fn issue_assign(ctx: &GriteContext, opts: &IssueAssignOptions) -> Result<IssueAssignResult, GriteError> {
    let _guard = LockGuard::acquire(ctx, &opts.issue_id, opts.acquire_lock)?;
    ...
    let ts = current_ts();
    let kind = if !opts.add.is_empty() {
        EventKind::AssigneeAdded { user: opts.add[0].clone() }
    } else if !opts.remove.is_empty() {
        EventKind::AssigneeRemoved { user: opts.remove[0].clone() }
    } else {
        return Err(GriteError::InvalidArgs("No assignees to add or remove".to_string()));
    };
...
/// Add a link.
pub fn issue_link(ctx: &GriteContext, opts: &IssueLinkOptions) -> Result<IssueLinkResult, GriteError> {
    let _guard = LockGuard::acquire(ctx, &opts.issue_id, opts.acquire_lock)?;
...
/// Add an attachment.
pub fn issue_attach(ctx: &GriteContext, opts: &IssueAttachOptions) -> Result<IssueAttachResult, GriteError> {
    let _guard = LockGuard::acquire(ctx, &opts.issue_id, opts.acquire_lock)?;
