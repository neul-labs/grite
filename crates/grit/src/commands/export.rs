...
pub fn run(cli: &Cli, format: ExportFormat, since: Option<String>) -> Result<(), GritError> {
    ...
    let since_filter = match since {
        Some(s) => {
            ...
            if s.len() == 64 {
                ...
                let ts: u64 = s.parse()
                    .map_err(|_| GritError::InvalidArgs(format!("Invalid since value: {}", s)))?;
    ...
    // Create .grit directory if needed
    let grit_export_dir = ctx.git_dir.parent().unwrap().join(".grit");
    std::fs::create_dir_all(&grit_export_dir)?;

    let (format_str, output_path, event_count) = match format {
        ExportFormat::Json => {
            let export = export_json(&store, since_filter)?;
            let output_path = grit_export_dir.join("export.json");
        ...
        ExportFormat::Md => {
            ...
            let output_path = grit_export_dir.join("export.md");
