...
pub fn run(cli: &Cli, cmd: ContextCommand) -> Result<(), GritError> {
    match cmd {
...
fn run_index(cli: &Cli, paths: Vec<String>, force: bool, pattern: Option<String>) -> Result<(), GritError> {
...
fn run_query(cli: &Cli, query: String) -> Result<(), GritError> {
    let ctx = GritContext::resolve(cli)?;
    let store = ctx.open_store()?;
...
fn run_show(cli: &Cli, path: String) -> Result<(), GritError> {
    let ctx = GritContext::resolve(cli)?;
    let store = ctx.open_store()?;

    let file_ctx = store.get_file_context(&path)?
        .ok_or_else(|| GritError::NotFound(format!("No context found for '{}'", path)))?;
...
fn run_project(cli: &Cli, key: Option<String>) -> Result<(), GritError> {
    let ctx = GritContext::resolve(cli)?;
    let store = ctx.open_store()?;

    if let Some(key) = key {
        let entry = store.get_project_context(&key)?
            .ok_or_else(|| GritError::NotFound(format!("Project context key '{}' not found", key)))?;
...
fn run_set(cli: &Cli, key: String, value: String) -> Result<(), GritError> {
    let ctx = GritContext::resolve(cli)?;
    let store = ctx.open_store()?;
    let wal = ctx.open_wal().map_err(|e| GritError::Internal(e.to_string()))?;
...
fn get_files_to_index(paths: &[String], pattern: &Option<String>) -> Result<Vec<String>, GritError> {
    ...
    let output = cmd.output()
        .map_err(|e| GritError::Internal(format!("Failed to run git ls-files: {}", e)))?;

    if !output.status.success() {
        return Err(GritError::Internal("git ls-files failed".to_string()));
    }
    ...
    if let Some(pat) = pattern {
        let glob = glob::Pattern::new(pat)
            .map_err(|e| GritError::InvalidArgs(format!("Invalid glob pattern: {}", e)))?;
